...
func TestSnellECHUTLSSessionCacheSharedByClone(t *testing.T) {
	...
	if config.uClientSessionCache != config.config.ClientSessionCache {
		t.Fatal("dedicated uTLS session cache was not installed in the base configuration")
	}
	...
	if cloned.uClientSessionCache != config.uClientSessionCache {
		t.Fatal("cloned configuration does not preserve the dedicated uTLS session cache")
	}
...
func TestSnellECHUTLSClientUsesDedicatedSessionCache(t *testing.T) {
	expectedCache := &recordingUTLSSessionCache{}
	config := &UTLSClientConfig{
		config:              &utls.Config{ServerName: "example.com", ClientSessionCache: &recordingUTLSSessionCache{}},
		id:                  utls.HelloChrome_Auto,
		uClientSessionCache: expectedCache,
	}
	...
	if len(expectedCache.getKeys) != 1 || expectedCache.getKeys[0] != "example.com" {
		t.Fatalf("dedicated uTLS session cache was not used: keys=%v", expectedCache.getKeys)
	}
}
