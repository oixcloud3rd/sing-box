...
if [ -z "$download_version" ]; then
  if [ "$download_beta" != "true" ]; then
    if [ -n "$GITHUB_TOKEN" ]; then
      latest_release=$(curl -s -H "Authorization: token ${GITHUB_TOKEN}" https://api.github.com/repos/oixcloud3rd/sing-box/releases/latest)
    else
      latest_release=$(curl -s https://api.github.com/repos/oixcloud3rd/sing-box/releases/latest)
    fi
    curl_exit_status=$?
    ...
    if [ -n "$GITHUB_TOKEN" ]; then
      latest_release=$(curl -s -H "Authorization: token ${GITHUB_TOKEN}" https://api.github.com/repos/oixcloud3rd/sing-box/releases)
    else
      latest_release=$(curl -s https://api.github.com/repos/oixcloud3rd/sing-box/releases)
    fi
    curl_exit_status=$?
    if [ $curl_exit_status -ne 0 ]; then
      exit $curl_exit_status
...
package_url="https://github.com/oixcloud3rd/sing-box/releases/download/v${download_version}/${package_name}"
