...
func TestBoundSelectorRejectsConnectionAfterSelectionChange(t *testing.T) {
	t.Parallel()

	leafA := &bindingTestParallelOutbound{bindingTestOutbound: newBindingTestOutbound("a")}
	leafB := newBindingTestOutbound("b")
	outboundManager := &bindingTestOutboundManager{strategies: map[adapter.Outbound]option.DestinationStrategy{
		leafA: option.DefaultDestinationStrategy(),
		leafB: option.DefaultDestinationStrategy(),
	}}
	selector := &Selector{
		Adapter:                      A.NewAdapter(C.TypeSelector, "", nil, nil),
		outbound:                     outboundManager,
		logger:                       log.NewNOPFactory().Logger(),
		outbounds:                    map[string]adapter.Outbound{"a": leafA, "b": leafB},
		interruptGroup:               interrupt.NewGroup(),
		interruptExternalConnections: true,
	}
	selector.selected.Store(adapter.Outbound(leafA))
	...
	require.ErrorIs(t, err, errSelectionChanged)
	require.Nil(t, conn)

	packetConn, destination, err := parallelDialer.ListenSerialNetworkPacket(
		context.Background(),
		M.Socksaddr{Fqdn: "example.com", Port: 443},
		nil,
		nil,
	...
	require.False(t, destination.IsValid())
}

func TestBoundURLTestRejectsConnectionAfterSelectionChange(t *testing.T) {
	t.Parallel()
	...
	group := &URLTestGroup{
		...
		outbounds:                    []adapter.Outbound{leafA, leafB},
		interruptGroup:               interrupt.NewGroup(),
		interruptExternalConnections: true,
	}
	group.selectedOutboundTCP.Store(adapter.Outbound(leafA))
	group.selectedOutboundUDP.Store(adapter.Outbound(leafA))
	urlTest := &URLTest{
		Adapter:  A.NewAdapter(C.TypeURLTest, "urltest", []string{N.NetworkTCP, N.NetworkUDP}, nil),
		outbound: outboundManager,
		logger:   log.NewNOPFactory().Logger(),
	...
	require.NoError(t, err)
	group.history.StoreURLTestHistory(leafB.Tag(), &adapter.URLTestHistory{Delay: 1})
	group.performUpdateCheck()
	require.Same(t, leafB, group.selectedOutboundTCP.Load())
